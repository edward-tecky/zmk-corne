...
- Confirmed safeguards are path-specific: unpair, keymap discard/layer delete/keymap reset, macro reset, factory reset, and prominent settings resets prompt the user; mutation hooks route through the unlock gate where configured. ZMK-SEC-019 records the remaining one-click Devtool and macro/combo actions instead of generalizing confirmation coverage.
